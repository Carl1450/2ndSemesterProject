...
public class CampsiteDAO {
    ...
    public List<Campsite> getCampsites() {
        String getCampsiteQ = "SELECT siteNo, section, road, [type], fee, maxpeople, deposit, price, effectiveDate "
                + "FROM ("
                + "    SELECT c.siteNo, c.section, c.road, c.[type], c.fee, cab.maxpeople, cab.deposit, pr.price, pr.effectiveDate, "
                + "           ROW_NUMBER() OVER (PARTITION BY c.siteNo ORDER BY pr.effectiveDate DESC) AS row_num "
                + "    FROM Campsite c "
    ...
    private List<Campsite> queryAvailableCampsites(Date startDate, Date endDate, boolean searchForCabin,
        ...
        String query = "SELECT c.siteNo, c.section, c.road, c.[type], c.fee, cab.maxpeople, cab.deposit, pr.price, pr.effectiveDate "
                ...
                + "LEFT JOIN Pitch p ON c.siteNo = p.siteNo "
                + "LEFT JOIN ("
                + "    SELECT campsiteSiteNo, price, effectiveDate, ROW_NUMBER() OVER (PARTITION BY campsiteSiteNo ORDER BY effectiveDate DESC) AS row_num "
                + "    FROM Price"
                + ") pr ON c.siteNo = pr.campsiteSiteNo AND pr.row_num = 1 "
                + "WHERE NOT EXISTS ("
    ...
    public boolean updateCampsiteBySiteNo(int siteNo, String section, String road, String type, int maxPeople,
                                          float deposit, float fee, float price) {
        Connection conn = DBConnection.getConnection(env);
